...
class _ProfileScreenState extends State<ProfileScreen> {
  ...
  void didChangeDependencies() {
    ...
    if (identical(_presenceProvider, presence)) return;
    _presenceProvider?.removeListener(_onPresenceProviderChanged);
    _presenceProvider = presence;
    _lastSeenRealtimeStatsVersion = presence.realtimeStatsVersion;
    _presenceProvider?.addListener(_onPresenceProviderChanged);
  }
  ...
  void _onPresenceProviderChanged() {
    final presence = _presenceProvider;
    if (!mounted || presence == null) return;
    final nextVersion = presence.realtimeStatsVersion;
  ...
  Widget build(BuildContext context) {
    ...
    final presence = context.read<PresenceProvider>();
    final inventoryItems = context
        .select<CommerceProvider, List<InventoryItemModel>>(
          (provider) => provider.inventoryItems,
        );
    final shopItems = context.select<CommerceProvider, List<ShopItemModel>>(
      (provider) => provider.shopItems,
    );
    final myPresence = user == null
        ? PresenceUtils.offline
        : context.select<PresenceProvider, String>(
            (provider) =>
                provider.presenceStatusForUser(user.id, isCurrentUser: true),
          );
    final t = Theme.of(context).textTheme;

    final activeItem = inventoryItems
    ...
    return RefreshIndicator(
      ...
      onRefresh: () async {
        await Future.wait([
          ...
          commerce.refreshInventory(),
          commerce.refreshInvoices(),
          _reloadPoints(),
        ]);
      },
      child: ListView(
                ...
                children: [
                  Row(
                    children: [
                      Semantics(
                        image: true,
                        label: 'Avatar de ${user?.username ?? 'l utilisateur'}',
                        child: SizedBox(
                          width: 58,
                          height: 58,
                          child: Stack(
                            ...
                            children: [
                              ...
                              Positioned(
                                ...
                                child: Semantics(
                                  button: true,
                                  label: 'Statut de presence',
                                  value: PresenceUtils.label(
                                    myPresence,
                                    isCurrentUser: true,
                                  ),
                                  hint: 'Modifier le statut de presence',
                                  child: InkWell(
                                    onTap: !isCurrentUser
                                        ? null
                                        : () => _showPresencePicker(
                      ...
                      IconButton(
                        tooltip: 'Ouvrir les reglages du profil',
                        ...
                        icon: const Icon(Icons.tune, color: AppColors.cFCFAFE),
                      ),
                    ],
                  ),
                    ...
                    children: [
                      Expanded(
                        child: Semantics(
                          ...
                          hint: 'Ouvre l ecran de modification du profil',
                          child: InkWell(
                      ...
                      Semantics(
                        ...
                        hint: 'Ouvre les options de partage du profil',
                        child: InkWell(
  ...
  Future<void> _showPresencePicker(
    ...
    await presence.updateMyPresenceStatus(selected);
  }
}

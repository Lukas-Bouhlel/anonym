  ...
  group('AuthProvider', () {
    ...
    setUp(() {
      ...
      when(
        ...
        (_) async => const UserModel(id: 2, username: 'a', email: 'a@test.dev'),
      );
      when(
        () => repository.signup(
        ...
        (_) async => const UserModel(id: 3, username: 'b', email: 'b@test.dev'),
      );
      when(
      ...
      when(
        ...
        (_) async => const UserModel(id: 4, username: 'c', email: 'c@test.dev'),
      );
      when(
    ...
    test('login and signup failures expose parsed error', () async {
      ...
      expect(
        await controller.signup(username: 'x', email: 'x@x', password: 'y'),
        isFalse,
      );
      expect(controller.errorMessage, isNotNull);
    });
    ...
    test(
      'requestRegisterCode stores info and retryAfter on dio failure',
      () async {
        final controller = AuthProvider(repository);
        addTearDown(controller.dispose);
        await Future<void>.delayed(Duration.zero);
    ...
    test(
      'confirmRegister can defer login until success dialog is closed',
      () async {
        when(() => repository.me()).thenThrow(
          DioException(
            requestOptions: RequestOptions(path: '/api/account'),
        ...
        final confirmed = await controller.confirmRegister(
          email: 'a@test.dev',
          code: '111111',
        ...
        controller.activatePendingRegistration();
    ...
    test(
      'reloadCurrentUser setUser logout and clearError mutate state',
      () async {
        final controller = AuthProvider(repository);
        addTearDown(controller.dispose);
        await Future<void>.delayed(Duration.zero);
        ...
        controller.setUser(
          const UserModel(id: 88, username: 'manual', email: 'm@test.dev'),
        );
    ...
    test('session expired handler resets state', () async {
      ...
      when(() => repository.setSessionExpiredHandler(any())).thenAnswer((
        invocation,
      ) {
      ...
      await Future<void>.delayed(Duration.zero);
      controller.setUser(
        const UserModel(id: 7, username: 'u', email: 'u@test.dev'),
      );
